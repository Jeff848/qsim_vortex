...
inline TYPE mul(TYPE a, TYPE b) {
  ...
  tempb.d = b;
  a1.u32 = tempa.u64 >> 32;
  a2.u32 = tempa.u64 & 0xffffffff;
  b1.u32 = tempb.u64 >> 32;
  b2.u32 = tempb.u64 & 0xffffffff;
  temp1.f = (a1.f * b1.f - a2.f * b2.f);
  temp2.f = (a1.f * b2.f - a2.f * b1.f);
  idouble ret;
  ret.u64 = ((uint64_t)(temp1.u32) << 32) | temp2.u32;
  return (ret.d);
}
...
void cpu_qsim(int num_states, int max_num_gates, int* num_qubits_arr, 
  int* gate_indexes_arr, TYPE* gate_matrix_arr, TYPE* states) {
  uint32_t q = 0;
  uint32_t m = 0;
  std::vector<TYPE> local_states(num_states);
  
  for (uint32_t g = 0; g < max_num_gates; g++) {
    // end of gate array
    ...
    uint32_t num_applications = num_states / num_data;
    for(uint32_t i = 0; i < num_applications; ++i) {
      ...
      for(uint32_t j = 0; j < num_data; ++j) {
          ...
          for(uint32_t l = 0; l < num_data; ++l) {
            TYPE temp_mul = mul(gate_matrix_arr[m + j * num_data + l], local_states[i * num_data + l]);
            temp = add(temp, temp_mul);
          }
...
int main(int argc, char *argv[]) {
  ...
  // std::srand(50);
  // Todo: rel add
  std::string filename = "/home/jm/vortex/tests/regression/qsim_cpu/temp.qasm";
  std::ifstream infile(filename);
  ...
  cpu_qsim(num_states, max_num_gates, h_num_q.data(), h_ind.data(), h_mat.data(), h_A.data());
